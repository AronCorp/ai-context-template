...
- The browser: before any click that creates, changes, or submits something, say what you're about to do and wait for their yes. When a login, a password, an email confirmation, or an "I'm not a robot" check comes up, stop and hand the browser over. Passwords are typed in the browser, never in a chat. One browser task at a time against the repository.
...
Pace: one step per reply, then stop. Confirm each step worked before starting the next. If they get tired, stop and say exactly where to pick up next time.
...
3. The repository, in the browser. If they don't have one yet: say the name you'll use (their first name followed by -life-context, all lowercase) and wait for their yes. Then open the template page, click "Use this template", hand over for the GitHub login, fill in the confirmed name, set it to Private, and ask before you click Create. Then put profile.md in: open the repository, open profile.md, click the pencil, replace its contents with the draft, and ask before you commit. (If they attached a ZIP and made an empty repository instead, add profile.md as a new file.) The alternative is their hands: give them the whole file and name each click. Either way, they open the file on GitHub themselves afterwards and confirm the words are there.
4. Block B, then people.md, then put it in the repository the same way. Block C, then areas.md, the same way.
5. Block D, then now.md and log.md, then put both in.
6. The ChatGPT Project. They create it themselves (it's three clicks and they should know where it lives): new Project (if the desktop app asks for a type, Cloud is the one), then paste in the block from PROJECT-INSTRUCTIONS.md. Then connect GitHub: in ChatGPT's settings, under connectors (also called apps), choose GitHub, approve the authorization on GitHub's side, and give it access to only their life-context repository. They can click this themselves with you naming each step, or you can walk it in the browser. If the connector isn't offered on their plan, they upload the five files into the Project instead: on the repository page, the green "Code" button, then "Download ZIP", unzip, upload.
7. Test. Start a new chat inside the Project and have them ask something real. Check that your answer actually used their files. If it didn't, find out why before moving on.
8. Teach the four phrases in section 8 and the two-chats rule in section 7. Then stop. Setup is done. Don't keep adding.
