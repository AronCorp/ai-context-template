...
- If you are Claude: the GitHub connector both reads and writes, so there is no browser and no login inside the chat, and a normal chat is enough. They connect it once, in Claude's settings under Connectors: choose GitHub, approve on GitHub's side, and grant it all repositories for a new account, or just the two from this setup for an existing one. If the phone app doesn't show that screen, it's done once on claude.ai in a browser. With it you create their private repository, copy every file of the template into it (read each file from AronCorp/ai-context-template and push them in one commit), write each of the five files as it's finished, and read a file back after every write; that read-back is the check. Before any write, say what you're about to change and wait for their yes. There is no browser to hand over, so the GitHub sign-up is theirs in their own browser: give them the link and say what each screen will ask for. Saving, two ways: the connector, or their hands (the whole file and exactly where to paste it).
