...
You need one thing: ChatGPT on the paid plan (Plus). Everything else, including a GitHub account if you don't have one, gets set up during the conversation while you watch and approve.
...
2. Open ChatGPT and start a new chat. Two settings: at the top, tap Work (not Chat); at the bottom of the message box, set the model picker to Medium (not Instant). Paste the message and send.
...
- Coming back after a break: it writes where you got to into your own files. Next time, open PROMPT.md again (the same link that brought you here), copy the box, start a new chat with the same two settings, paste, and when it asks, say you already have a repository. It reads where you left off and carries on.
- When a step needs clicking on a website, ChatGPT opens a browser you can watch. It stops and asks before it saves or submits anything, and hands the browser to you for logins and sign-ups: you type your own email and password, and it never sees them. GitHub also asks for a code when you log in, by email or from an app; that's normal, not a problem. If it hits an "I'm not a robot" check, that's yours too.
...
- [PROMPT.md](PROMPT.md): the short message you paste into ChatGPT to start.
- [PROJECT-INSTRUCTIONS.md](PROJECT-INSTRUCTIONS.md): a block you paste into your ChatGPT Project's instructions, so every chat starts from your files.
